...
const PACKAGE_ID = "modules/dfreds-hidden-token-effects";
...
const config = Vite.defineConfig(({ command, mode }): Vite.UserConfig => {
    ...
    if (command === "serve") {
        ...
        fs.writeFileSync(
            "./styles/dfreds-hidden-token-effects.css",
            `/** ${message} */\n`,
        );
        fs.writeFileSync(
            "./dfreds-hidden-token-effects.mjs",
    ...
    return {
        base:
            command === "build" ? "./" : `/modules/dfreds-hidden-token-effects/`,
        publicDir: "static",
        define: {
        ...
        build: {
            ...
            lib: {
                name: "dfreds-hidden-token-effects",
            ...
            rollupOptions: {
                ...
                output: {
                    assetFileNames: "styles/dfreds-hidden-token-effects.css",
                    chunkFileNames: "[name].mjs",
                    entryFileNames: "dfreds-hidden-token-effects.mjs",
                    manualChunks: {
        ...
        server: {
            ...
            open: false,
            proxy: {
                "^(?!/modules/dfreds-hidden-token-effects/)":
    ...
    return {
        ...
        writeBundle(outputOptions) {
            ...
            const lockFile = path.resolve(
                outDir,
                "dfreds-hidden-token-effects.lock",
            );
            fs.rmSync(lockFile);
        },
    };
